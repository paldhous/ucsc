...
Not so many years ago, data was hard to obtain. Often data journalists would have to painstakingly compile their own datasets from paper records, or make specific requests for electronic databases using freedom of information laws -- which were not passed in Britain until 2000 (and in Scotland, 2002).

The Internet has changed the game. Today, many government and other public databases can now be queried online, and the results of those searches downloaded. Other datasets can be downloaded in their entirety.
...
In our first class we considered specific data portals relecang to science reporters. Often, however, your starting point in searching for data will be Google. Simply combining a few keywords in a Google search with “data” or “database” may be enough to find what you need, but it can be worth focusing your queries using Google’s [advanced search](http://www.google.com/advanced_search):

![](./img/class2_1.jpg)

(Source: [Google](http://www.google.com/advanced_search))

The options to search by `site or domain` and `file type` can be especially useful when looking for data. For example, the [U.S. Geological Survey](http://www.usgs.gov/) is the best source of data on earthquakes and seismic risk, so when searching for this information, specifying the domain `usgs.gov` would be a good idea. You can make the domains as narrow or broad as you like: `.ac.uk`, for instance, would search the sites of all British academic institutions using that top-level domain; `city.ac.uk` would search the web pages of this university only.
...
Also find out whether the database allows “wildcards,” symbols such as `*` or `%` that can be dropped into your search to obtain results with variations on a word or number. For example, in a database that may contain entries in both British and American English, you might use a wildcard to be sure you obtain results with both spelling variants, such as `colo*r`.
...
Downloading an entire database, where this is allowed, frees you from the often-limited options given on an online advanced search form: You can then upload the data into your own database software, and query it in any way that you want. We will learn how to do this in a subsequent class.
...
To illustrate how this works, go to the [ISRCTN clinical tral registry](http://www.isrctn.com/), and then navigate to the [advanced search page](http://www.isrctn.com/editAdvancedSearch). Enter "cancer" under `Condition` and note that more than 1600 trials are returned.
...
Websites like the ISRCTN clinical tral registry are not expressly designed to be searched by manipulating their urls, but some organisations make their data available through APIs that can be queried by constructing a url in a similar way. This allows websites, apps and data journalists to call in specific chunks of data as required.

To see how this works, go to the U.S. Geological Survey's [Earthquake Archive Search & URL Builder](http://earthquake.usgs.gov/earthquakes/search/), where we will search for all earthquakes with a [magnitude](http://www.geo.mtu.edu/UPSeis/magnitude.html) of 3 or greater that occured witin 1,000 kilometres of the geographic center of the United Kingdom, which [this site](http://tools.wmflabs.org/os/coor_g/?pagename=Centre_points_of_the_United_Kingdom&params=SD4157566760_region%3AGB_scale%3A25000) tells us lies at a latitude of `54.093409` degrees and a longitude of `-2.89479` degrees (the minus sign denotes a longitide West of the Greenwich Meridian). We will initially ask for the data in a format called [GeoJSON](http://geojson.org/) (a variant of JSON). Enter `1990-01-01T00:00:00` under `Start` for `Date & Time` boxes so that we obtain all recorded earthquakes from the beginning of 1990 onwards. Make the `End` date and time blank. The search form should look like this:
...
```
http://comcat.cr.usgs.gov/fdsnws/event/1/query?starttime=1990-01-01T00:00:00&latitude=54.093409&longitude=-2.89479&maxradiuskm=1000&minmagnitude=3&format=geojson&orderby=time
```
...
Now remove the `-asc` and replace `geojson` in the url with `csv`. The data should now download as a CSV file.
...
Using programming languages such as [Python](https://www.python.org/) or [Ruby](https://www.ruby-lang.org/en/), it is possible to write scripts that will pull data down from many webpages, or query web search forms to download an entire database piece by piece.

Web scraping by coding is beyond the scope of this class. However, the underlying process is simple to understand: Identify the patterns you would need to follow if collecting the data manually, then write a program to automate the process and write the results to a data file.
...
That often means experimenting to reveal the most efficient way of exposing all of the data you require. Let's say we are interested in gathering all available data on clinical trials taking place in the U.K. There are several available data sources, which include two sites we have considered earlier, [ClinicalTrials.gov](http://clinicaltrials.gov/) and the [ISRCTN clinical tral registry](http://www.isrctn.com/), plus the World Health Organization's [International Clinical Trials Registry Platform](http://apps.who.int/trialsearch/default.aspx).

To demonstrate webscraping, we will consider the [U.K Clinical Trials Gateway](http://www.ukctg.nihr.ac.uk/trials.aspx), which is supposed to incorporate data from ClinicalTrials.gov and the ISRCTN registry (although at the time of writing only the former appeared to be shown at searches on the site).

Enter `*a* OR *e* OR *i* OR *o* OR *u*` into the gateway's search box. Through experimentation, I have found that this search seems to return all available trials in the database. Think about why this works: `*` is a wildcard, and using it in this way with `OR` Boolean operators asks the database to return all trials with a keyword containing a vowel. (I have also run checks to verify that the number of trials returned from this search is broadly consistent with a search at ClinicalTrials.gov for all trials located in the U.K.)
...
This is what we will now do, using software that allows non-programmers to scrape data from the web. These tools include [OutWit Hub](https://www.outwit.com/) and the Windows-only [Helium Scraper](http://www.heliumscraper.com/en/index.php?p=home). However, in today's class, we will use [**Import.io**](https://import.io/), to extract data from a sample of 100 links from the U.K Clinical Trials Gateway.

Download those pages from [here](.data/UKCTG.zip), move to your Desktop and unzip the folder.
...
Here, all of the numbers represent the same variable, and there is a column for each year. This is good for people to read, but data analysis and visualization software does not always play well with data in this format.

So if you receive "wide" data, you may need to covert it to "long" format, shown here:
...
Now we will use Open Refine to process and clean data on grants from the U.S. federal government awarded to the University of California, Berkeley, in 2013. The data was obtained from [USASpending.gov](http://usaspending.gov/), and you can download it [from here](./data/ucb_grants/ucb_grants.csv).

Create a new project and import the data as before. Again, Open Refine should recognize that the data is in a CSV file

Once the data has loaded, notice that entries recognized as numbers or dates will be green, those treated as strings of text will be black.
...
Often we may need to convert fields to text, numbers or dates. For example, click on the button for `starting_date` and select `Edit cells>common transforms>To date` and see that it changes from a string of text to a date.
