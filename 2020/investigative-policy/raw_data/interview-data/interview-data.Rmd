# Interviewing data: R, RStudio, and the tidyverse to find story leads and trends
...
Download the data for these classes from [here](data/interview-data.zip), unzip the folder and place it on your desktop. It contains the following files:
...
We can use the **[dplyr](https://dplyr.tidyverse.org/)** package to  process and analyze data, using these basic operations:
...
We will now start filter and sort the data according to the doctors' locations, and the types of disciplinary actions they faced.
...
```{r}
# license revocations for doctors based in California, by year
...
Now we will work with the larger data on doctors in California prescribing opioids under Medicare Part D, learn how to **join** data in different data frames, and make some simple charts to help make sense of your data.
...
Remember, when summarizing data, any `NA` values will cause the summary to be calculated as `NA`, unless you specify that nulls should be removed with `na.rm = TRUE`.
...
This code first stated that the `provider_summary` data would be used for the chart in the `ggplot` function, then specificied that values for the number of prescriptions should be mapped to the X, or horizontal axis. We didn't need to specify a variable for the Y axis for a histogram, because that is automatically a count of the number of records (here the number of medical providers) in each bin.

The code customizes the width of the bins, to 50 prescriptions, changes the X axis range between 0 and 3,000 so we don't plot outliers with many more prescriptions than this, and uses `labels` from the **scales** package to plot numbers with comma separators for thousands. As we learned last quarter, `theme_minimal` is one of the built-in themes in **ggplot2**, which gives a basic chart with grid lines on a white background.
...
Scatterplots are good for seeing how one variable relates to one another, and also for spotting outliers in data. Here we would expect the total cost of the drugs prescribed to increase with the total number of prescriptions. A scatterplot allows us to confirm whether that is the case, and allows us to spot interesting outliers.
...
Remember the words of caution: There may be some story leads here, but we would have to find out more about each doctor's practice and patients to determine whether outliers in terms of prescriptions and costs are justified.
...
ca_discipline_npi <- left_join(ca_discipline, npi_license, by = c("license" = "plicnum"))

test <- ca_discipline_npi %>%
  group_by(license) %>%
  summarize(npis = n_distinct(npi)) %>%
  arrange(-npis)
...
The data returned reveals some of the limitations of the `npi_license` data. Some of the doctors have not been joined to an `npi` code. Others have been joined to multiple `npi` codes.
...
Individual doctors may appear multiple times in the data if they have more than one disciplinary action on their file, because a join will be made to each individual action. And given the issues with the NPI data, you would need to check the identity of the doctor at both the [NPI lookup](https://npiregistry.cms.hhs.gov/) and [California professional licensing](https://www.breeze.ca.gov/) sites to confirm that a genuine match has been made.

You can also search for the disciplinary records and look at public documents at the latter site.

Naga Thota, near the top of this list, surrendered his California medical license on March 22, 2017. [This public document](https://www2.mbc.ca.gov/BreezePDL/document.aspx?path=%5cDIDOCS%5c20170315%5cDMRAAAFC8%5c&did=AAAFC170315180454982.DID) reveals that he was charged with prescribing excessive quantities of opioids to an addicted patient with whom he also engaged in a sexual relationship. In addition to disciplinary action from the Medical Board of California, he faced a criminal investigation by the federal Drug Enforcement Adminstration. In March 2017, Thota was [sentenced to two years](https://www.sandiegouniontribune.com/news/courts/sd-me-thota-sentencing-20170306-story.html) to two years in a federal prison.
...
```{r}
# join disciplinary action data to the opioid prescription data
provider_summary_actions_extra <- anti_join(provider_summary_actions_2, provider_summary_actions, by = c("nppes_provider_last_org_name"
...
Whenever making an join in this way, it crucial to verify through further reporting that the individuals matched are the same person. There will almost certainly be false matches in the data!
