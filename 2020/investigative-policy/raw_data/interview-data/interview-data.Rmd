...
The data returned reveals some of the limitations of the `npi_license` data. Some of the doctors have not been joined to an `npi` code, while others appear to have more than one `npi` code (this is why there are 11,363 rows in the joined data, and only 7,651 in the `ca_discipline` data).
...
Individual doctors may appear multiple times in the data if they have more than one disciplinary action on their file, because a join will be made to each individual action. And given the issues with the NPI data, you would need to check the identity of the doctor at both the [NPI lookup](https://npiregistry.cms.hhs.gov/) and [California professional licensing](https://www.breeze.ca.gov/) sites.

We can search for the disciplinary records and look at public documents at the latter.
